// Sistema de carrinho melhorado - VERSÃO FINAL FUNCIONAL
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Recuperar carrinho do cookie com tratamento robusto de erro
    try {
      if (req.cookies['simple_cart']) {
        try {
          ...
          console.log(`[API Simple Cart Add] 📦 Carrinho do cookie recuperado com ${sessionCart.items.length} itens`);
      ...
      console.log('[API Simple Cart Add] ⚠️ Erro geral no cookie, criando carrinho vazio:', parseError.message);
      sessionCart = { items: [] };
    }
    ...
    try {
      ...
      try {
        ...
        if (cookieSize > 3700) {
          ...
          // IMPORTANTE: Criar versão limitada APENAS para o cookie
          // Mas manter o sessionCart original com todos os itens
          finalCart = {
            ...superSafeCart,
            items: limitedItems,  // Items limitados APENAS para o cookie
          ...
          // CRÍTICO: Manter sessionCart com TODOS os itens originais
          // O sessionCart não deve ser alterado, apenas o que vai pro cookie
          ...
          console.log(`[API Simple Cart Add] ℹ️ SessionCart mantém todos os ${sortedItems.length} itens originais`);
          ...
          // Cookie não está no limite, usar todos os itens
          finalCart = superSafeCart;
        }
        ...
        console.log(`[API Simple Cart Add] ✅ Cookie do carrinho salvo com sucesso (${finalCookieValue.length} bytes)`);
          // GARANTIR QUE TODOS OS ITENS SEJAM RETORNADOS NA RESPOSTA
        // Mesmo que o cookie tenha limitações, a resposta deve conter todos os itens
        const fullCart = {
          ...sessionCart,
        ...
        // Salvar no armazenamento do servidor para persistir todos os produtos
        try {
          ...
          // Gerar ou reutilizar ID de sessão
          ...
          // Verificar se já existe um carrinho e integrar os itens atuais
          const existingCart = global.serverCartStorage[sessionId] || { items: [] };
          
          // Mapa para controle dos produtos por identificador único
          const productsMap = {};
          
          // Primeiro adicionar todos os itens da sessão atual
          ...
          // Adicionar itens existentes que não estão na sessão atual
          if (Array.isArray(existingCart.items)) {
            existingCart.items.forEach(item => {
              if (item && item.productId) {
                const key = `${item.productId}_${item.variationId || 0}`;
                // Se o produto já existe, atualizamos apenas se for uma versão mais recente
                if (!productsMap[key] || 
                    (item.cartKey && productsMap[key].cartKey && 
                     parseInt(item.cartKey.split('_')[2] || 0) > 
                     parseInt(productsMap[key].cartKey.split('_')[2] || 0))) {
                  productsMap[key] = { ...item };
                }
              }
            });
          ...
          // Adicionar cookie de sessão aos headers existentes
          const cookies = res.getHeader('Set-Cookie') || [];
          const cookieArray = Array.isArray(cookies) ? cookies : [cookies];
          cookieArray.push(sessionCookie);
          res.setHeader('Set-Cookie', cookieArray);
          
          console.log(`[API Simple Cart Add] 💾 Carrinho completo armazenado no servidor com ID: ${sessionId} (${allItems.length} itens)`);
        } catch (storageError) {
          console.error('[API Simple Cart Add] ❌ Erro ao salvar no armazenamento do servidor:', storageError);
        }
        ...
        // CRÍTICO: Sempre retornar TODOS os itens na resposta, independente do cookie
        const responseData = {
          success: true,
          ...
          cart: {
            // SEMPRE enviar TODOS os itens do sessionCart completo
            items: sessionCart.items,  // Este é o array completo com todos os itens
            ...
            hasMoreItems: finalCart.hasMoreItems || false,
            itemsInCookie: finalCart.itemsInCookie || sessionCart.items.length,
            // Adicionar flag para indicar que esta resposta contém todos os itens
  ...
  // IMPORTANTE: Verificar flag para garantir que itens sejam sempre adicionados
  if (req.body.append_to_server_storage && sessionId && global.serverCartStorage?.[sessionId]) {
    const existingItems = global.serverCartStorage[sessionId].items || [];
    ...
    if (existingItemIndex >= 0) {
      ...
      existingItems[existingItemIndex].quantity = (
        parseInt(existingItems[existingItemIndex].quantity || 1) + 
        parseInt(quantity || 1)
      );
    } else {
      // Adicionar novo produto sem substituir os existentes
      existingItems.push(sessionItem);
    }
    
    // Atualizar o armazenamento global com todos os itens
    global.serverCartStorage[sessionId] = {
      ...global.serverCartStorage[sessionId],
      items: existingItems,
      timestamp: Date.now()
    ...
    console.log(`[API Simple Cart Add] ✅ Item adicionado ao armazenamento servidor: ${existingItems.length} itens totais`);
  } else {
    // Comportamento padrão (substituir ou inicializar)
    global.serverCartStorage[sessionId] = {
      items: [sessionItem], // Pode substituir itens existentes
      timestamp: Date.now()
    };
  }
}
