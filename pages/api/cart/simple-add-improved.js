...
export default async function handler(req, res) {
  ...
  try {
    ...
    }    // RECUPERAR CARRINHO DO COOKIE COM TRATAMENTO DE ERRO
    let sessionCart = { items: [] };
    
    // Recuperar carrinho do cookie se existir
    try {
    ...
    try {
      ...
      try {
        ...
        // SALVAR CARRINHO COMPLETO NO ARMAZENAMENTO PERSISTENTE
        try {
          // Gerar ou reutilizar session ID
          if (!sessionId) {
            sessionId = `cart_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
            console.log('[API Simple Cart Add] 🆔 Novo session ID gerado:', sessionId);
          }
          
          // Salvar carrinho completo no sistema de armazenamento
          await saveCompleteCart(sessionId, fullCart);
          console.log('[API Simple Cart Add] 💾 Carrinho completo salvo no armazenamento persistente');
          
          // Definir cookie do session ID
          const sessionCookie = `cartSessionId=${sessionId}; Path=/; Max-Age=86400; SameSite=Lax`;
          res.setHeader('Set-Cookie', [cartCookie, sessionCookie]);
          
        } catch (storageError) {
          console.log('[API Simple Cart Add] ⚠️ Erro ao salvar no armazenamento:', storageError.message);
          // Continuar mesmo com erro de armazenamento, pois o cookie ainda funciona
        }
