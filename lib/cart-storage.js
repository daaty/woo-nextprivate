...
function getCart(sessionId) {
  try {
    ...
    console.log(`[Cart Storage] ✅ Carrinho recuperado para sessão ${sessionId} (${cartData.length || 0} itens)`);
    return cartData;
  } catch (error) {
 ...
 */
function saveCart(sessionId, cartData) {
  try {
    if (!sessionId) {
    ...
    console.log(`[Cart Storage] ✅ Carrinho salvo para sessão ${sessionId} (${cartData.items?.length || 0} itens)`);
    return true;
  } catch (error) {
