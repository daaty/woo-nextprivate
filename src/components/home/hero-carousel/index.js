...
const HeroCarousel = ({heroCarousel}) => {
    ...
    const nextSlide = () => {
        ...
        <div className="banner flex flex-col sm:flex-row justify-between overflow-hidden relative mt-5 sm:mt-0">
            <div className="banner-img sm:w-8/12 w-full">
                {
                    heroCarousel.map( ( item, index ) => {
                        const opacity = ( activeIndex === index || 1 === heroCarousel.length ) ? 'opacity-100' : 'opacity-0';
                        return (
                            <div key={item?.id} className={`${opacity} banner-img-container absolute top-0 left-0 w-full h-full`}>
                                <img
                ...
                <div className="slider-button absolute bottom-4 right-4 bg-white bg-opacity-70 rounded-md p-1">
            ...
            <div className="banner-content pt-10 sm:pt-0 px-4 sm:px-10 sm:w-4/12 bg-white sm:bg-transparent z-10">
                ...
                <Link href={`/category/${heroCarousel[activeIndex]?.slug}/`}>
                    <a className="banner-content__link text-gray-700 inline-block mt-2 hover:text-blue-600">+ Explore</a>
                </Link>
            </div>
        </div>
    )
